...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  useEffect(() => {
    const subscription = watch((value, { name, type }) => {
      if (name === 'title') {
        const newSlug = value.title?.trim().replaceAll(' ', '_').toLowerCase();
  ...
  const onNewTag = (key: any) => {
    if (key.code === 'Space' && newTagValue && newTagValue !== '') {
      const newTags = [...getValues('tags'), newTagValue]
  ...
  const onSubmit = async (form: FormData) => {
    ...
    try {
      const { data } = await tesloApi({
        url: '/admin/products',
        method: form._id ? 'PUT' : 'POST', // SI tenemos un _id acutalizar si no crear
        data: form
      })
      console.log(data);
      if (!form._id) {
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <TextField
              ...
              {...register('price', {
                required: 'Este campo es requerido',
                minLength: { value: 0, message: 'Valor mínimo 0' }
              })}
              error={!!errors.price}
